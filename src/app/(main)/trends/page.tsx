  ...
  return (
    <span
      className={cn(
        ...
        isPositive &&
          "bg-positive-lighter text-positive-main dark:bg-positive-darker/30 dark:text-positive-textDark",
        !isPositive &&
          !isNeutral &&
          "bg-negative-lighter text-negative-main dark:bg-negative-darker/30 dark:text-negative-textDark",
        isNeutral &&
          "bg-light-secondary text-light-muted-text dark:bg-dark-secondary dark:text-dark-muted-text",
      )}
    >
      {isPositive ? (
      ...
      >
        ...
        <div className="p-5">
          ...
          <div className="space-y-1">
            <p className="text-sm font-medium text-light-muted-text dark:text-dark-muted-text">
              {title}
            </p>
...
const HeatmapCell: React.FC<{
  ...
  return (
    <div className="text-center">
      ...
      {label && (
        <p className="text-xs text-light-muted-text dark:text-dark-muted-text mt-2 truncate">
          {label}
        </p>
      )}
    </div>
          ...
          <div className="flex items-center gap-2">
            <span
              className={cn(
                "text-lg font-bold",
                isTop
                  ? "text-positive-main dark:text-positive-textDark"
                  : "text-negative-main dark:text-negative-textDark",
              )}
            >
...
const TrendsDashboard: React.FC = () => {
  ...
  const { data: filterDictionary, isLoading: filterLoading } =
    useFilterDictionary();
  const {
    data: processedData,
    rawData,
  ...
  const selectedFilterLabels = useMemo(() => {
    ...
    if (filters.academicYearId) {
      const year = filterDictionary.academicYears.find(
  ...
  const metrics = useMemo(() => {
    if (!processedData || !rawData) {
      return {
        ...
        subjectPerformers: [] as {
          id: string;
          name: string;
          rating: number;
          responses: number;
        }[],
        academicYearTrends: [] as {
          year: string;
          rating: number;
          responses: number;
        }[],
    ...
    const academicYearSemesterTrends =
      processedData.academicYearSemesterTrends || [];
    const academicYearDepartmentTrends =
    ...
    const avgFacultyRating =
      facultyData.length > 0
        ? facultyData.reduce((a, b) => a + b.averageRating, 0) /
          facultyData.length
        : 0;
    const aboveAvgFaculty = facultyData.filter(
      (f) => f.averageRating >= avgFacultyRating,
    ).length;

    // Subject above average
    const validSubjectRatings = subjectData.filter(
      (s) => s.overallAverageRating !== null,
    );
    const avgSubjectRating =
      validSubjectRatings.length > 0
        ? validSubjectRatings.reduce(
            (a, b) => a + (b.overallAverageRating || 0),
            0,
          ) / validSubjectRatings.length
        : 0;
    const aboveAvgSubjects = subjectData.filter(
      (s) => (s.overallAverageRating || 0) >= avgSubjectRating,
    ).length;

    // Top and bottom performers (faculty) - No random trends, use real data
    const sortedFaculty = [...facultyData].sort(
    ...
    const topPerformers = sortedFaculty.slice(0, 5).map((f) => ({
      ...
      trend: Number(
        (
          ((f.averageRating - avgFacultyRating) / avgFacultyRating) *
          100
        ).toFixed(1),
      ),
    }));
    const bottomPerformers = sortedFaculty
      .slice(-5)
      .reverse()
      .map((f) => ({
        id: f.facultyId,
    ...
    const sortedSubjects = [...subjectData]
      ...
      .sort(
        (a, b) => (b.overallAverageRating || 0) - (a.overallAverageRating || 0),
      );
    ...
    const semesterTrends = academicYearSemesterTrends
      .map((trend) => {
        // Get the latest academic year's data for each semester
        const latestData =
          trend.academicYearData[trend.academicYearData.length - 1];
        return {
    ...
    academicYearDepartmentTrends.forEach((trend) => {
      const totalRating = trend.departmentData.reduce(
        (sum, d) => sum + d.averageRating * d.responseCount,
        0,
      );
      const totalResponses = trend.departmentData.reduce(
    ...
    const latestYearTrend =
      academicYearDepartmentTrends[academicYearDepartmentTrends.length - 1];
    const departmentComparison =
      latestYearTrend?.departmentData.map((d) => ({
    ...
    if (academicYearTrends.length >= 2) {
      const current =
        academicYearTrends[academicYearTrends.length - 1]?.rating || 0;
      const previous =
        academicYearTrends[academicYearTrends.length - 2]?.rating || 0;
      healthTrend =
        previous > 0
    ...
    if (departmentComparison.length > 1) {
      ...
      if (maxRating - minRating > 1) {
        const lowDept = departmentComparison.find(
          (d) => d.rating === minRating,
        );
        insights.push({
          priority: "medium",
  ...
  const handleFilterChange = (
    key: keyof AnalyticsFilterParams,
  ...
  const handleExportCSV = useCallback(() => {
    ...
    const summaryData = [
      ...
      },
      {
        ...
        Value: metrics.totalSubjects.toString(),
        Unit: "subjects",
      },
      {
        Metric: "Health Trend",
        Value: `${metrics.healthTrend > 0 ? "+" : ""}${metrics.healthTrend.toFixed(1)}`,
        Unit: "%",
      },
    ];
    ...
    <div className="h-screen bg-light-muted-background dark:bg-dark-background flex overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="space-y-6 p-6">
          ...
          <div className="relative bg-light-background dark:bg-dark-muted-background p-6 rounded-lg border border-light-secondary dark:border-dark-secondary overflow-hidden hover:shadow-md transition-shadow duration-200">
                ...
                <div>
                  ...
                  <p className="text-sm text-light-muted-text dark:text-dark-muted-text mt-1">
                    ...
                    {selectedFilterLabels.length > 0 && (
                      <span className="ml-2">
                        • Filtered by:{" "}
                        {selectedFilterLabels.map((label, i) => (
                          <span
                            key={label}
                            className="font-medium text-primary-main"
                          >
                            {label}
                            {i < selectedFilterLabels.length - 1 ? ", " : ""}
                          </span>
                        ))}
                      </span>
                    )}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-center px-4 py-2 bg-light-muted-background dark:bg-dark-noisy-background rounded-lg">
                    <p className="text-2xl font-bold text-warning-main">
                      {
                        metrics.insights.filter((i) => i.priority === "high")
                          .length
                      }
                    </p>
                    <p className="text-xs text-light-muted-text dark:text-dark-muted-text">
                      ...
                      {showExportMenu && (
                        ...
                        >
                          ...
                          >
                            ...
                            Export as Excel
                            <span className="ml-auto text-xs text-light-muted-text dark:text-dark-muted-text">
                              Multi-sheet
                            </span>
                          </button>
                        </motion.div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <ExecStatCard
                ...
                sparklineData={metrics.departmentComparison.map(
                  (d) => d.rating,
                )}
              />
              <ExecStatCard
                ...
                subtitle={
                  metrics.healthTrend !== 0
                    ? `${metrics.healthTrend > 0 ? "+" : ""}${metrics.healthTrend.toFixed(1)}% from last period`
                    : "All time"
                }
                ...
                trendLabel="%"
                sparklineData={metrics.academicYearTrends.map(
                  (t) => t.responses,
                )}
                    ...
                    {metrics.semesterTrends.length > 0 ? (
                      ...
                      <div className="text-center py-8 text-light-muted-text dark:text-dark-muted-text">
                        ...
                        <p className="text-xs mt-1">
                          Select an academic year to see trends
                        </p>
                      </div>
                    )}
                  </Card>
                  ...
                  <motion.div variants={itemVariants}>
                    <Card className="p-5 h-full bg-light-background dark:bg-dark-muted-background border border-light-secondary dark:border-dark-secondary">
                      ...
                      <PerformersTable
                        ...
                        type="top"
                        onItemClick={(id, name) =>
                          drillDown.openFacultyPanel(id, name)
                        }
                      />
                    </Card>
                  </motion.div>
